...
const TokenBudgetParameter = ({
  ...
  useEffect(() => {
    const parameter = parameters.find((p) => p.name === 'budget_tokens');
    if (parameter) {
  ...
  const handleTokenBudgetChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const parameter = updatedParameters.find((p) => p.name === 'budget_tokens');
    if (parameter) {
      parameter.value = Number(e.target.value);
    }
    else {
      updatedParameters.push({ name: 'budget_tokens', type: 'int', value: Number(e.target.value) });
    }
    ...
    <div className="special-parameter start">
      ...
      <div className="body">
        <div className="token-budget-value align-top">
          <WrappedInput type="number" value={tokenBudget} onChange={(e) => handleTokenBudgetChange(e)} />
        </div>

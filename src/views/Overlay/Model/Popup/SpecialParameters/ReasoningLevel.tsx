...
const ReasoningLevelParameter = ({
  ...
  const handleReasoningLevelChange = (level: string) => {
    ...
    const parameter = updatedParameters.find((p) => p.name === 'reasoning_effort');
    if (parameter) {
      parameter.value = level;
    }
    else {
      updatedParameters.push({ name: 'reasoning_effort', type: 'string', value: level });
    }
      ...
      <div className="title align-top">
        <div className="title-left">
          ...
          <InfoTooltip
            side="left"
            content={
              t('models.reasoningLevelTooltip')
            }
          >
        ...
        <div className="description">
          {t('models.reasoningLevelDescription')}
        </div>
      </div>
      <div className="body">
        <div className="reasoning-level-btn-group">
          <button className={`btn ${reasoningLevel === 'low' ? 'active' : ''}`} onClick={() => handleReasoningLevelChange('low')}>
            <span>Low</span>
          </button>
          <button className={`btn ${reasoningLevel === 'medium' ? 'active' : ''}`} onClick={() => handleReasoningLevelChange('medium')}>
            <span>Medium</span>
          </button>
          <button className={`btn ${reasoningLevel === 'high' ? 'active' : ''}`} onClick={() => handleReasoningLevelChange('high')}>
            <span>High</span>
          </button>
        </div>
      </div>
